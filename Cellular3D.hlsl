#ifndef CEULLULAR_3D_INCLUDED
#define CEULLULAR_3D_INCLUDED
...
// periodic variant
float2 CellularNoise(float3 P, float period) {
#define K 0.142857142857 // 1/7
#define Ko 0.428571428571 // 1/2-K/2
#define K2 0.020408163265306 // 1/(7*7)
#define Kz 0.166666666667 // 1/6
...
#define jitter 1.0 // smaller jitter gives more regular pattern
	...
	float3 p = permute(mod(Pi.x + float3(-1.0, 0.0, 1.0), period));
	float3 p1 = permute(mod(p + Pi.y - 1.0, period));
	float3 p2 = permute(mod(p + Pi.y, period));
	float3 p3 = permute(mod(p + Pi.y + 1.0, period));

	float3 p11 = permute(mod(p1 + Pi.z - 1.0, period));
	float3 p12 = permute(mod(p1 + Pi.z, period));
	float3 p13 = permute(mod(p1 + Pi.z + 1.0, period));

	float3 p21 = permute(mod(p2 + Pi.z - 1.0, period));
	float3 p22 = permute(mod(p2 + Pi.z, period));
	float3 p23 = permute(mod(p2 + Pi.z + 1.0, period));

	float3 p31 = permute(mod(p3 + Pi.z - 1.0, period));
	float3 p32 = permute(mod(p3 + Pi.z, period));
	float3 p33 = permute(mod(p3 + Pi.z + 1.0, period));
