#ifndef CEULLULAR_2X2X2_INCLUDED
#define CEULLULAR_2X2X2_INCLUDED
...
float2 CellularNoise2x2x2(float3 P) {
...
// periodic variant
float2 CellularNoise2x2x2(float3 P, float period) {
#define K 0.142857142857 // 1/7
#define Ko 0.428571428571 // 1/2-K/2
#define K2 0.020408163265306 // 1/(7*7)
#define Kz 0.166666666667 // 1/6
...
#define jitter 0.8 // smaller jitter gives less errors in F2
	...
	float4 Pfx = Pf.x + float4(0.0, -1.0, 0.0, -1.0);
	float4 Pfy = Pf.y + float4(0.0, 0.0, -1.0, -1.0);
	float4 p = permute(mod(Pi.x + float4(0.0, 1.0, 0.0, 1.0), period));
	p = permute(mod(p + Pi.y + float4(0.0, 0.0, 1.0, 1.0), period));
	float4 p1 = permute(mod(p + Pi.z, period)); // z+0
	float4 p2 = permute(mod(p + Pi.z + 1.0, period)); // z+1
	float4 ox1 = frac(p1*K) - Ko;
	float4 oy1 = mod7(floor(p1*K))*K - Ko;
	float4 oz1 = floor(p1*K2)*Kz - Kzo; // p1 < 289 guaranteed
	float4 ox2 = frac(p2*K) - Ko;
